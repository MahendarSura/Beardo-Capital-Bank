    ...
    <div className="max-w-7xl mx-auto px-6 py-12 font-sans bg-white">
      ...
      <section className="bg-white shadow rounded-md p-8 grid grid-cols-1 md:grid-cols-4 gap-8 w-full">
        {[
          ...
          { icon: "💰", title: "Loans", text: "Home loans, personal loans, and car loans with competitive rates." },
          { icon: "📱", title: "Digital Banking", text: "Manage accounts and payments anywhere, anytime with our mobile app." },
        ].map((item, i) => (
          <div key={i} className="flex flex-col items-start hover:shadow-lg cursor-pointer transition p-6 border border-gray-100 rounded">
            <div className="text-5xl mb-4 text-[#E80000]">{item.icon}</div>
            <h3 className="text-2xl font-bold mb-2 text-[#E80000]">{item.title}</h3>
            <p>{item.text}</p>
          </div>
        ))}
      </section>
        ...
        <div className="text-center space-y-8">
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="rounded shadow hover:shadow-lg cursor-pointer overflow-hidden border border-gray-100">
                <img src={offer.img} alt={offer.title} className="w-full h-48 object-cover" />
                <div className="p-6 text-left">
                  <h3 className="text-2xl font-semibold mb-2 text-[#E80000]">{offer.title}</h3>
                  <p>{offer.desc}</p>
                </div>
              </div>
            ))}
            ...
            {[
              {
                ...
                name: "Rajesh Kumar",
                comment: "Beardo Capital Bank has transformed how I manage my finances. Their app is super easy and customer support is fantastic!",
              },
              {
                img: "https://randomuser.me/api/portraits/women/44.jpg",
                name: "Sneha Patil",
                comment: "The loan process was smooth and quick. I got my funds in less than 24 hours!",
              },
              {
